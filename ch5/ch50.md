...
- Waterfall assumes requirements can be mostly defined up front, so teams progress through phases in sequence with limited backtracking.
- Agile assumes requirements will evolve, so teams deliver in short iterations, gather feedback frequently, and adapt plans continuously.

In practice, one is usually better than the other depending on context:

- Waterfall is often better when requirements are stable, scope is well understood, and strict documentation, approvals, or compliance are required.
- Agile is often better when uncertainty is high, user feedback is critical, and the team needs to learn and adjust quickly while delivering incremental value.
