...
gh repo delete my-project-with-semver --yes
```
...
Create a new empty repository in Github and name it `my-project-with-semver`.

```{code-cell} bash
:tags: ["remove-input","remove-output"]
gh repo create my-project-with-semver --public
```
...
git branch -M main
git remote add origin https://github.com/mcallara/my-project-with-semver.git
  ...
  release:
    ...
    steps:
      ...
      - name: Run Semantic Release
        env:
          GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: uv run semantic-release version' >> .github/workflows/release.yml
```
    ...
    steps:
      ...
      - name: Run Semantic Release
        ...
        run: |
          ...
          uv lock
          git add uv.lock
          uv run semantic-release -v version' >> .github/workflows/release.yml
```
