...
:class: hint
- If we check the "Actions" tab of the repository, we can see that a new workflow run has been triggered by our commit. We can click on the workflow run to see the details of the execution. Under the "Artifacts" section, we can see that the distribution files have been uploaded as artifacts of the workflow. We can download them if needed.
```
