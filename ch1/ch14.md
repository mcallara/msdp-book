...
Finally, once your feature it's merged, you usually don't want to keep the feature branch around. To delete a branch, use the `git branch -d <branch-name>` command. For example, to delete the `new-branch-name` branch:

```{code-cell} bash
git brach-d new-branch-name
```
