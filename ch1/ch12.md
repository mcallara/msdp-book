...
```{admonition} PowerShell note
:class: note
In PowerShell, use `Get-ChildItem -Force` to list all files including hidden ones.
```
...
```{admonition} PowerShell note
:class: note
In PowerShell, use `Remove-Item -Recurse -Force .git`
```
...
```{admonition} PowerShell note
:class: note
In PowerShell, use `New-Item -ItemType File file1.txt` to create a new file and `Add-Content file1.txt "This is the first line."` to add content to the file.
```
...
```{admonition} Know more: pushing all branches at once
:class: note
Instead of pushing each branch one by one, you can push every local branch in a single command using the `--all` option:
...
- If your local `main` has no unique commits, Git performs a fast-forward update.
- If local and remote have diverged, Git creates a merge commit (or asks you to resolve conflicts first).
...
```{admonition} Know more: git pull vs git fetch
:class: note
- `git pull` is a shortcut for `git fetch` followed by integrating changes (usually a merge).
- `git fetch` only updates your remote-tracking references (for example `origin/main`) and does not change your current local branch.

The default `pull` behavior can be changed to rebase instead of merge, or to only allow fast-forward updates. You can set this behavior on a per-pull basis or as a default for all repositories:
- One-time: `git pull --rebase origin main` to rebase instead of merge.
- One-time strict sync: `git pull --ff-only origin main` to allow only fast-forward updates.
- Default for all repositories: `git config --global pull.rebase true` (rebase by default).
- Default fast-forward policy: `git config --global pull.ff only` (reject pulls that need a merge commit).

Use `git pull` when you want to sync immediately. Use `git fetch` when you want to inspect incoming changes first, then merge or rebase manually.
```
