...
:class: note
In PowerShell you should use: `New-Item -ItemType File notes.txt`
```
...
```{admonition} macOS and PowerShell note
:class: note
`ls --help` is a GNU/Linux style flag and will not work the same way on macOS or in PowerShell.
- On macOS, use `man ls`.
- In PowerShell, use `Get-Help ls`.
```
