...
Let's open VSC, create a new directory `ch2` and use the option "Open Folder" to open this directory. Now, create a new file `time_intervals.py`.

## Querying Copilot with the inline chat 
As soon as we create the new Python file, we see the message "Press Ctrl+I to ask Github Copilot Chat to do something". 
