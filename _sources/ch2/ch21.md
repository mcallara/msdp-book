...
jupytext:
  formats: md:myst
  text_representation:
    extension:```{admonition}Prompt
Write a Python module that divides expenses among friends for trips or outings. The app should:
- Accept user input expenses, specifying the payer and the total amount in the format [("Person1":[34,2,3,45]),("Person2":[3,4,26]),...].
...
First, proposes a code snippet that creates two sample dataframes with a common column. If we hover over the code, we can see that there will be a symbol with three dots that will allow us to access the option "Insert into New File".
