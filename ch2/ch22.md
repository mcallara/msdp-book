...
To test this, we need some functions, so I prompted Copilot with the following:
```
As an expert software engineer, refactor this code so that it has some functions in it.
```
And it returns:
...
if __name__ == "__main__":
    main()
```
Now we can use functions breakwpoints wich halt execution when entering a specified function.

1. In the Debug view under Breakpoints, click on "Add Function Breakpoint" and type the name of the function, `calculate_share`.
2. When you run the debugger, it will pause every time `calculate_share` is invoked, allowing you to inspect the initial state of the function and its parameters.
...
To see this in action, prompt your favorite LLM to handle the exception raised whenever you pass a non-numeric value to amount in `add_expense` function. In my case, it returns:    
```{code-block} python
def add_expenses(balances):
    while True:
        cmd = input("Add expense? (y/n) ").lower()
